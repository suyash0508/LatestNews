intent(
  ...
  "who are you?",
  reply(
    'NewsMaster is a news app which is completely voice controlled. You can search latest news by categories, terms and sources. Try saying: "Give me the latest technology news."'
  )
);

// const NEWS_API_KEY = "33ab0373fbc34cfa8b095498c17e3faf";
...
// Weather Information
intent(
  `h(tell me|wat's|show me|) (the|) (temperature|weather) (of|in) $(place* (.+))`,
  //p.place.value == delhi
  (p) => 
  {
      ...
      //TO STORE FORECAST INFORMATION
      let forecastInfo;

    //api.request has the url & call back function with three parameters(err,response,body)
    //WEATHER API CALL
    api.request(WEATHER_URL, (err, response, body) => {
      console.log("Hi");
    
    //all the parse json object from body is destructed into these defined variables;
      ...
      //json object forecastInfo defined all the key value pairs;
      forecastInfo = { 
        name: location.name,
    ...
    //api.request for air quality index
...
intent(
  ...
  (p) => {
    let NEWS_API_URL = `https://newsapi.org/v2/top-headlines?apiKey=${NEWS_API_KEY}&country=india`;
    if (p.C.value) {
    ...
    async function fetchdata() {
      ...
      if (p.C.value) {
        p.play(`(Here are the|) (latest|recent) articles on ${p.C.value}.`);  //giving news by category
      } 
      
      else {
        p.play(`Here are the (latest|recent) articles.`);  //If no category then give all the news (latest news)
      }
...
//function for confirmation to play all the headlines or not;
//  "Yes" or "No";
